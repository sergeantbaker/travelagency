...
if (isset($_POST['bewerk2'])) {
	...
	header("Location: admin.php?content=contact/melding_ov&ID=$ID");
}
...
<tr onclick="location.href='index.php?content=contact/melding_ov&ID=<?php echo $row[5]; ?>'">
	<th>ID: <?php echo $row[5] ?></th>
	<th>Gebruikersnaam: <?php echo $row[6] ?></th>
	<th>Naam: <?php echo $row[7] ?> <?php echo $row[8] ?></th>
<tr>
	<td>Status: <strong><?php echo $row[3];?></strong></td> 
	<td>Soort: <strong><?php echo $row[4];?></strong></td> 
...
<input name="bewerk2" type="submit" id="bewerk2" value="Extra melding" class= "btn btn-primary" onclick="klikbewerk()">
...
?>
	<tr onclick="location.href='index.php?content=contact/melding_ov&ID=<?php echo $row[2]; ?>'">
